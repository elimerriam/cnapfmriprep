...
seventprep preprocess "$ROOT/bids" "$ROOT/derivatives/seventprep" \
  --config "$CONFIG" \
  --subject "$SUBJECT" \
  --session "$SESSION" \
  --work-dir "$ROOT/work/preprocess"

printf '\nDerivatives: %s\n' "$ROOT/derivatives/seventprep"
printf 'QC reports:  %s\n' "$ROOT/derivatives/seventprep/sub-$SUBJECT/ses-$SESSION/figures"
